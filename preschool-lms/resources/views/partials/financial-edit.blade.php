...
<div class="grid grid-cols-1 md:grid-cols-2 gap-8">
    ...
    <div>
        ...
        <div class="mb-4">
            <select name="financier" id="financier" class="mt-1 block w-full p-3 rounded-md border-gray-300 shadow-sm">
                <option value="" disabled {{ old('financier', $financialData->financier ?? '') == '' ? 'selected' : '' }}>
                    Select Financier
                </option>
                <option value="Parents" {{ old('financier', $financialData->financier ?? '') == 'Parents' ? 'selected' : '' }}>Parents
                </option>
                <option value="Relatives" {{ old('financier', $financialData->financier ?? '') == 'Relatives' ? 'selected' : '' }}>Relatives
                </option>
                <option value="Guardian" {{ old('financier', $financialData->financier ?? '') == 'Guardian' ? 'selected' : '' }}>Guardian
                </option>
                <option value="Myself" {{ old('financier', $financialData->financier ?? '') == 'Myself' ? 'selected' : '' }}>Myself
                </option>
                <option value="Others" {{ old('financier', $financialData->financier ?? '') == 'Others' ? 'selected' : '' }}>Others
                </option>
            </select>
            @error('financier')
        ...
        <div class="mb-4">
            <input type="text" name="contact_number" id="contact_number"
                ...
                value="{{ old('contact_number', $financialData->contact_number ?? '') }}">
    ...
    <div class="">
        ...
        <div id="relative-info-container">
            @php
                // Exploding the data into arrays for edit view
                ...
                @for ($i = 0; $i < $maxRows; $i++)
                    <div class="relative-entry flex flex-col border-b hover:bg-gray-300 bg-gray-200">
                        ...
                        <div class="relative-entry-details px-4 py-3 hidden" id="relative-entry-details-{{ $i }}">
                            <div class="px-4 py-2">
                                <input type="text" name="relative_names[{{ $i }}]"
                                    ...
                                    placeholder="Name" value="{{ $relativeNames[$i] ?? '' }}">
                            </div>
                            <div class="px-4 py-2">
                                <input type="text" name="relationships[{{ $i }}]"
                                    class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:ring-indigo-500 focus:border-indigo-500 transition duration-200 ease-in-out"
                                    placeholder="Relationship" value="{{ $relationships[$i] ?? '' }}">
                            </div>
                            <div class="px-4 py-2">
                                <input type="text" name="position_courses[{{ $i }}]"
                                    class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:ring-indigo-500 focus:border-indigo-500 transition duration-200 ease-in-out"
                                    placeholder="Position/Course" value="{{ $positionCourses[$i] ?? '' }}">
                            </div>
                            <div class="px-4 py-2">
                                <input type="text" name="relative_contact_numbers[{{ $i }}]"
                                    class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:ring-indigo-500 focus:border-indigo-500 transition duration-200 ease-in-out"
                                    placeholder="Contact Number" value="{{ $relativeContactNumbers[$i] ?? '' }}">
                            </div>
                        </div>
